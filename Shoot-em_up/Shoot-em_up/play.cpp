...
Play::Play(SDL_Renderer* renderer) {
    ...
    textSurface = TTF_RenderText_Solid(font, "Continue", 8, textColor);
...
void Play::Render(SDL_Renderer* _renderer) {
    ...
    if (textTexture) {
        ...
        SDL_FRect dst = {buttonRect.x, buttonRect.y, (float)tw, (float)th};
        SDL_RenderTexture(_renderer, textTexture, nullptr, &dst);
    }
}

void Play::Press(SDL_Renderer* _renderer) {
    buttonRect = {475, 525, 100, 40};
    ...
    if (textTexture) {
        float tw, th;
        SDL_GetTextureSize(textTexture, &tw, &th);
        SDL_FRect textRect = {buttonRect.x, buttonRect.y, (float)tw, (float)th};
